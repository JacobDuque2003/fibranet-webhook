...
app.use((req, res, next) => {
  ...
  res.json = (body) => {
    if (body && typeof body === 'object' && body.mensaje && !body.message) {
      body.message = body.mensaje;
    }
...
async function leerComprobante(imageUrl) {
  try {
    ...
    console.log(`👁️ [VISION] Descargando imagen desde: ${imageUrl.substring(0, 120)}`);
...
async function subirImagenDrive(imageUrl, nombre, cedula, comprobante) {
  try {
    ...
    if (!imgResponse.ok) {
      return { exito: false, error: `No se pudo descargar imagen para Drive: HTTP ${imgResponse.status}` };
    }
...
async function verificarMercatelyAPI() {
  return {
    estado: 'ok',
    mensaje: 'No requerida: usando {{last_message_media_url}} desde Mercately Chatbot',
...
function verificarMercately() {
  ...
  const minutosTranscurridos = Math.floor((new Date() - ultimoPingMercately) / 60000);
  ...
  if (minutosTranscurridos < 360) {
    return {
      estado: 'advertencia',
      mensaje: `Último ping hace ${Math.floor(minutosTranscurridos / 60)} hora(s)`,
      ultimo_ping: ultimoPingMercately.toISOString(),
      minutos_transcurridos: minutosTranscurridos
    };
  }
...
app.get('/health', async (req, res) => {
  ...
  res.json({
    ...
    servicios: {
      railway: {
        estado: 'ok',
        mensaje: 'Servidor v6.1 funcionando',
        version: '6.1',
      ...
      mercately_api: mercatelyApi,
      mercately_chatbot: mercately
    },
    sesiones: {
...
app.get('/status', async (req, res) => {
  ...
  const colorEstado = (e) =>
    e === 'ok' ? '#22c55e' :
    e === 'advertencia' ? '#f59e0b' :
    e === 'desconocido' ? '#6b7280' :
    '#ef4444';
  ...
  const servicioHtml = (nombre, icono, servicio, detalle = '') => `
    <div class="servicio" style="border-left:4px solid ${colorEstado(servicio.estado)}">
      <div class="servicio-header">
        <div class="servicio-nombre"><span class="servicio-icono">${icono}</span>${nombre}</div>
        <span class="badge" style="background:${colorEstado(servicio.estado)}20;color:${colorEstado(servicio.estado)}">
          ${iconoEstado(servicio.estado)} ${servicio.estado.toUpperCase()}
        </span>
      </div>
      <div class="servicio-mensaje">${servicio.mensaje}</div>
      ${servicio.tiempo_respuesta_ms !== undefined ? `<div class="servicio-detalle">Tiempo de respuesta: ${servicio.tiempo_respuesta_ms}ms</div>` : ''}
      ${detalle ? `<div class="servicio-detalle">${detalle}</div>` : ''}
      ${servicio.error ? `<div class="servicio-detalle" style="color:#ef4444">Error: ${servicio.error}</div>` : ''}
    </div>`;
...
.sesiones{background:rgba(96,165,250,0.05);border:1px solid rgba(96,165,250,0.2);border-radius:12px;padding:16px;margin-top:16px;text-align:center;color:#94a3b8;font-size:13px}
.sesiones strong{color:#60a5fa}
</style>
</head>
<body>
<div class="container">
  <div class="header">
    ...
    <div class="subtitle">Panel de Estado del Sistema · v6.1</div>
    <div class="estado-general">${todos_ok ? '✅ TODO OPERATIVO' : '⚠️ REVISAR SERVICIOS'}</div>
  </div>
  <div class="servicios">
    ${servicioHtml('Railway (Servidor)', '🚂', railway, `Node ${process.version} · Puerto ${process.env.PORT || 3000}`)}
    ${servicioHtml('MikroWisp (ISP)', '🌐', mikrowisp)}
    ${servicioHtml('Google Drive', '📁', drive)}
    ${servicioHtml('Google Vision (OCR)', '👁️', vision)}
    ${servicioHtml('Mercately API', '🔑', mercatelyApi)}
    ${servicioHtml('Mercately (Chatbot)', '💬', mercately, mercately.ultimo_ping ? `Último contacto: ${new Date(mercately.ultimo_ping).toLocaleString('es-EC', { timeZone: 'America/Guayaquil' })}` : '')}
  </div>
...
app.get('/', (req, res) => {
  res.json({ estado: 'FibraNet Webhook activo ✅', version: '6.1' });
});
...
app.post('/cliente/buscar', async (req, res) => {
  try {
    ...
    res.status(500).json({
      encontrado: false,
      transferir: true,
      mensaje: '⚠️ Error del sistema. Un asesor le atenderá.'
    });
  }
});
...
app.post('/cliente/deuda', async (req, res) => {
  try {
    ...
    if (!cedula) {
      return res.json({ mensaje: '⚠️ Error: no se identificó al cliente. Por favor escriba "pago" para empezar de nuevo.' });
    }

    const resultado = await buscarClientePorCedula(cedula);

    if (!resultado.exito) {
      return res.json({ mensaje: '❌ No se encontró información del cliente. Escriba "pago" para empezar de nuevo.' });
    }
...
app.post('/pago/info', async (req, res) => {
  try {
    ...
    if (!cedula) {
      return res.json({ mensaje: '⚠️ Error: no se identificó al cliente. Escriba "pago" para empezar de nuevo.' });
    }
    ...
    if (facturas === 0) {
      return res.json({
        deuda: 0,
        mensaje: '✅ No tiene deudas pendientes. ¡Está al día! 🎉'
      });
    }
...
app.post('/pago/comprobante', async (req, res) => {
  try {
    const { cedula } = req.body;
    const imagen_url = req.body.imagen_url || req.body.last_message_media_url;

    console.log('📸 [COMPROBANTE] ===== INICIO =====');
    console.log(`📸 [COMPROBANTE] Cédula: "${cedula}"`);
    console.log(`📸 [COMPROBANTE] Imagen URL: "${imagen_url}"`);

    if (!cedula) {
      return res.json({
        activado: false,
        mensaje: '⚠️ Error: no se identificó al cliente. Escriba "pago" para empezar.'
      });
    }
    ...
    if (!imagen_url || !/^https?:\/\//i.test(imagen_url)) {
      return res.json({
        activado: false,
        mensaje: '⚠️ No pude recibir la imagen del comprobante. Por favor envíe nuevamente la foto desde WhatsApp.'
      });
    }
    ...
    console.log(`👤 [COMPROBANTE] Cliente: ${nombreCompleto} | Deuda: $${deuda.toFixed(2)}`);
    ...
    if (!ocr.exito) {
      return res.json({
        activado: false,
        mensaje: '⚠️ No pude leer el comprobante. Por favor envíe una foto más clara y nítida. 📸'
      });
    }
    ...
    if (!exitosa) {
      return res.json({
        activado: false,
        mensaje: '❌ La transferencia no aparece como exitosa en el comprobante.\n\nVerifique que la transferencia fue aprobada e intente de nuevo.'
      });
    }
    ...
    if (!monto) {
      return res.json({
        activado: false,
        mensaje: '⚠️ No pude leer el monto del comprobante. Por favor envíe una foto más clara. 📸'
      });
    }
    ...
    res.status(500).json({
      activado: false,
      mensaje: '⚠️ Error procesando el comprobante. Un asesor lo revisará manualmente.'
    });
  }
});
...
app.post('/cliente/plan', async (req, res) => {
  try {
    ...
    if (!cedula) {
      return res.json({ mensaje: '⚠️ Error: no se identificó al cliente.' });
    }
    ...
    if (!servicio) {
      return res.json({ mensaje: '❌ No se encontró información del servicio.' });
    }
...
app.listen(PORT, () => console.log(`🚀 FibraNet Webhook v6.1 corriendo en puerto ${PORT}`));
