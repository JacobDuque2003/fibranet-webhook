...
// 🎓 NUEVA EN v5.4: Función centralizada para buscar cliente por cédula
// La usamos en TODOS los endpoints para mantener consistencia
...
function extraerDatosComprobante(texto) {
  const montoPatterns = [
    /\$\s*(\d+[.,]\d{2})/, /MONTO[:\s]+\$?\s*(\d+[.,]\d{2})/i,
    /VALOR[:\s]+\$?\s*(\d+[.,]\d{2})/i, /TOTAL[:\s]+\$?\s*(\d+[.,]\d{2})/i,
    /(\d+[.,]\d{2})\s*USD/i
  ];
  let monto = null;
  ...
  const comprobantePattterns = [
    ...
    /(?:TRX|TXN|REF|OP)[:\s#-]*(\d+)/i, /\b(\d{8,12})\b/
  ];
...
app.get('/health', async (req, res) => {
  ...
  res.json({
    ...
    servicios: {
      railway: { estado: 'ok', mensaje: 'Servidor v5.4 funcionando', version: '5.4', node: process.version, uptime_segundos: Math.floor(process.uptime()) },
...
app.get('/status', async (req, res) => {
  const [mikrowisp, drive, vision] = await Promise.all([verificarMikroWisp(), verificarGoogleDrive(), verificarGoogleVision()]);
  const mercately = verificarMercately();
  const railway = { estado: 'ok', mensaje: `v5.4 · Uptime: ${Math.floor(process.uptime() / 60)} min` };
  const todos_ok = mikrowisp.estado === 'ok' && drive.estado === 'ok' && vision.estado === 'ok' && (mercately.estado === 'ok' || mercately.estado === 'desconocido');
...
app.get('/', (req, res) => {
  res.json({ estado: 'FibraNet Webhook activo ✅', version: '5.4' });
});
...
app.post('/cliente/buscar', async (req, res) => {
  try {
    ...
    console.log(`✅ [BUSCAR] Cliente: ${nombreCompleto} (cédula: ${cliente.cedula})`);
...
app.post('/cliente/deuda', async (req, res) => {
  try {
    ...
    if (!cedula) return res.json({ mensaje: '⚠️ Error: no se identificó al cliente. Por favor escribe "pago" para empezar de nuevo.' });

    const resultado = await buscarClientePorCedula(cedula);

    if (!resultado.exito) {
      console.log(`❌ [DEUDA] Cliente no encontrado: ${cedula}`);
      return res.json({ mensaje: '❌ No se encontró información del cliente. Escribe "pago" para empezar de nuevo.' });
    }
    ...
    if (facturas === 0) {
      return res.json({ deuda: 0, mensaje: `✅ *Estimado(a) cliente*, no tienes deudas pendientes.\n\n👤 ${nombreCompleto}\n\n¡Gracias por mantener tu pago al día! 🎉` });
    }
...
// ════════════════════════════════════════════════════════
// 🎓 INFO DE PAGO - v5.4 RECIBE CÉDULA
// ════════════════════════════════════════════════════════
app.post('/pago/info', async (req, res) => {
  try {
    ...
    if (!cedula) return res.json({ mensaje: '⚠️ Error: no se identificó al cliente. Escribe "pago" para empezar de nuevo.' });
...
// ════════════════════════════════════════════════════════
// 🎓 PROCESAR COMPROBANTE - v5.4 RECIBE CÉDULA
// ════════════════════════════════════════════════════════
app.post('/pago/comprobante', async (req, res) => {
  try {
    ...
    if (!cedula) return res.json({ activado: false, mensaje: '⚠️ Error: no se identificó al cliente. Escribe "pago" para empezar.' });
...
app.post('/cliente/plan', async (req, res) => {
  try {
    ...
    if (!cedula) return res.json({ mensaje: '⚠️ Error: no se identificó al cliente. Escribe "pago" para empezar.' });
...
// ════════════════════════════════════════════════════════
// 🎓 REPORTE TÉCNICO - v5.4 RECIBE CÉDULA
// ════════════════════════════════════════════════════════
...
// ════════════════════════════════════════════════════════
// 🎓 CAMBIO DE CLAVE - v5.4 RECIBE CÉDULA
// ════════════════════════════════════════════════════════
...
app.listen(PORT, () => console.log(`🚀 FibraNet Webhook v5.4 corriendo en puerto ${PORT}`));
